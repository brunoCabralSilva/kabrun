...
export default function Notes() {
  ...
  // const { session, userEmail, setShowMessage } =  useContext(contexto);
  ...
  const updateValue = async () => {
    // const player: any = await getPlayerByEmail(session.id, email, setShowMessage);
    // if (player) {
    //   player.data[type] = text;
		// 	await updateDataPlayer(session.id, email, player.data, setShowMessage);
    // } else setShowMessage({ show: true, text: 'Jogador não encontrado! Por favor, atualize a página e tente novamente' });
  };

  return(
    <div className="flex flex-col w-full pr-2 h-90vh overflow-y-auto">
