...
export default function Notifications() {
  const { session, listNotification, setListNotification, setShowMessage } = useContext(contexto);
  
  const remNotFromCache = async(list: any[]) => {
    const listNotif = listNotification.filter((listNot: any) => JSON.stringify(listNot) !== JSON.stringify(list));
          ...
          listNotification.map((listNot: any, index: number) => (
            ...
            ? <div key={index} className="border-2 pt-1 px-1 border-white mb-2">
              ...
              <div className="flex w-full gap-2 px-3 mb-3">
                <button
                  type="button"
                  onClick={ () => removeNotification(session.id, listNot.message, setShowMessage) }
